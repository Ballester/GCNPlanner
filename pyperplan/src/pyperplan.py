...
def _search(task, search, heuristic, use_preferred_ops=False):
    ...
    if heuristic:
        if use_preferred_ops:
            solution, expansions = search(task, heuristic, use_preferred_ops)
        else:
            solution, expansions = search(task, heuristic)
    else:
        solution, expansions = search(task)
...
def search_plan(domain_file, problem_file, search, heuristic_class,
                use_preferred_ops=False, model=None):
    """
    ...
    if not heuristic_class is None:
        print(heuristic_class)
        if 'GCN' in heuristic_class.__name__:
            heuristic = heuristic_class(task, domain_file, model)
        else:
            heuristic = heuristic_class(task)

    search_start_time = time.clock()
    if use_preferred_ops and isinstance(heuristic, heuristics.hFFHeuristic):
        solution, expansions = _search(task, search, heuristic, use_preferred_ops=True)
    else:
        solution, expansions = _search(task, search, heuristic)

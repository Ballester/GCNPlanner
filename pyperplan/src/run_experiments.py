...
random_heuristic = HEURISTICS['random']
...
for task in tasks:
    ...
    # solution, expansions = search_plan(domain_file, task, search, random_heuristic,
    #                                    use_preferred_ops=use_preferred_ops)
    # random_expansions.append(expansions)
...
# print(np.mean(random_expansions), np.std(random_expansions))
print(np.mean(lmcut_expansions), np.std(lmcut_expansions))
with open('results/' + args.domain + '.txt', 'w') as fid:
    ...
    lmcut_result = "%.2f %.2f\n" % (np.mean(lmcut_expansions), np.std(lmcut_expansions))
    fid.write(gcn_result)
    fid.write(blind_result)
    fid.write(lmcut_result)
